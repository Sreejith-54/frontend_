...
const StudentManagement = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.post(`/api/admin/delete/students/${id}`);
      fetchStudents();
    } catch (e) {
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const response = await api.post("/api/admin/student-bulk-upload", {
    ...
    <div>
      ...
      {selectedSection ? (
        <>
          ...
          {/* LIST */}
          <h4 style={{ marginBottom: '10px' }}>Student List ({filteredStudents.length})</h4>
